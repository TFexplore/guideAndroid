...
public abstract class HttpRequest<T extends HttpRequest<?>> {
    ...
    private HashMap<String,Object> mQuerys=new HashMap<>();
        ...
        for (Field field : fields) {
            ...
            try {
                ...
                if (field.isAnnotationPresent(Query.class)) {
                    Log.d("request", "createRequest: -------------"+String.valueOf(value));
                    mQuerys.put(key, value);
                    continue;
                }

                addHttpParams(params, key, value, type);
                Log.d("http", "createCall: -----------------------");
            } catch (IllegalAccessException e) {
    ...
    protected Request createRequest(String url, String tag, HttpParams params, HttpHeaders headers, BodyType type) {
        ...
        if (!mQuerys.isEmpty()){
            for (String key:mQuerys.keySet()){
                Object value = mQuerys.get(key);
